...
public class Screen extends JPanel implements Runnable {
    ...
    public void startGame(){
        ...
        AbstractTower machineGunTower = new MachineGunTower(20, 100, 2, 1, 10, 1D, 1D, "res/tower/tower3.png");
                ...
                if (e.getXOnScreen() >= 50 * 23 && e.getXOnScreen() <= 50 * 24) {
                    ...
                    if (e.getXOnScreen() >= 1180 && e.getXOnScreen() <= 1320) {
                        if(player.getMoney() >= 50) {
                            ...
                            if (towerMap[towerChoosedX][towerChoosedY].equals(towerList[1])) { // machine
                                System.out.println("number update: " + towerMap[towerChoosedX][towerChoosedY].getNumberUpdate());
                                player.setMoney(player.getMoney() - 50); // money for update
                                if (towerMap[towerChoosedX][towerChoosedY].getNumberUpdate() == 1) { // update 1st
                                    towerMap[towerChoosedX][towerChoosedY] = new MachineGunTower(20, 100, 2, 3, 15, 1D, 1D, "res/tower/tower2.png");
                                }
                            }
                            if (towerMap[towerChoosedX][towerChoosedY].equals(towerList[2])) { //snipe
                                System.out.println("number update: " + towerMap[towerChoosedX][towerChoosedY].getNumberUpdate());
                                player.setMoney(player.getMoney() - 50); // money for update
                                if (towerMap[towerChoosedX][towerChoosedY].getNumberUpdate() == 1) { //update 1st
                                    towerMap[towerChoosedX][towerChoosedY] = new SniperTower(20, 1, 6, 12, 15, 1D, 1D, "res/tower/tower2.png");
                                }
                            }
                        }
                    }
